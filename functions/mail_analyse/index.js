...
function analyseItem(item, uid){
	...
	let body = removeHTMLTags(item.body);
	result.analysis.headline = analyze(item.headline);
...
exports.handle = (e, context) => {
	...
	co(function* (){
		...
		for(let key of keys){
			...
			let analysisResult = analyseItem(item, key);
